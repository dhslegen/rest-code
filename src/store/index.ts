  ...
  actions: {
    parseRcsFile(content: string) {
      ...
      if (!validation.isValid) {
        ...
        ElMessage.error(
          `文件校验失败：发现 ${errorCount} 个错误${warningCount > 0 ? `, ${warningCount} 个警告` : ""}`,
        );
    ...
    validateRcsContent(content: string): {
      isValid: boolean;
      errors: Array<{
        line: number;
        message: string;
        ...
        if (line.startsWith("/")) {
          ...
          if (!domainName || domainName.trim() === "") {
            ...
            errors.push({
              line: lineNumber,
              message:
                "领域名称不符合大写字母开头的驼峰命名法（如：User、OrderItem）",
              severity: "error",
            });
          }
          ...
          if (apiPath && apiPath.trim() !== "" && !apiPathRegex.test(apiPath)) {
            errors.push({
              line: lineNumber,
              message:
                "API路径格式错误，只允许字母、数字、下划线、连字符和路径参数{xxx}，如：/users、/{id}、/users/{userId}/profile",
              severity: "error",
            });
          }
          ...
          if (!operation || operation.trim() === "") {
            ...
            errors.push({
              line: lineNumber,
              message:
                "操作名称不符合小写字母开头的驼峰命名法（如：create、getUserById）",
              severity: "error",
            });
          }
          ...
          if (
            contract &&
            contract.trim() !== "" &&
            !contractRegex.test(contract)
          ) {
            errors.push({
              line: lineNumber,
              message:
                "参数契约格式错误，应为 @xxx?yyy%num%$str>zzz 的组合（如：@、?、%id、>）",
              severity: "error",
            });
          }
